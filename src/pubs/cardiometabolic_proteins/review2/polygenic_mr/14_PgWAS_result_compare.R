...
poly_mr <- fread("analyses/pub/cardiometabolic_proteins/review2/polygenic_mr.txt")
...
poly_mr <- unique(poly_mr[,.(PRS, GWAS, Target, UniProt, Gene, PolyMR=exp(mr_estimate.consensus), PolyMR.L95=exp(mr_L95.consensus),
                             PolyMR.U95=exp(mr_U95.consensus), PolyMR.P=mr_pval.consensus, PolyMR.pleiotropy=pleiotropy_pval)])
...
poly_mr[map, on = .(PRS, GWAS), endpoint := endpoint]
...
comp <- merge(comp, poly_mr, by=c("PRS", "GWAS", "endpoint", "Target", "UniProt", "Gene"), all.x=TRUE)
...
comp[, PolyMR := format_hr(PolyMR)]
comp[, PolyMR.95CI := sprintf("[%s, %s]", format_hr(PolyMR.L95), format_hr(PolyMR.U95))]
comp[, PolyMR.P := my_format_pval(PolyMR.P)]
comp[, PolyMR.pleiotropy := my_format_pval(PolyMR.pleiotropy)]

comp[HR == "NA", c("HR", "HR.95CI", "HR.P", "HR.FDR") := ""]
comp[MR == "NA", c("MR", "MR.95CI", "MR.P", "MR.FDR", "MR.pleiotropy", "colocalization") := ""]
comp[PolyMR == "NA", c("PolyMR", "PolyMR.95CI", "PolyMR.P", "PolyMR.pleiotropy") := ""]
...
# Arrange columns
comp <- comp[, .(Target, UniProt, Gene, PRS, PRS.Beta, PRS.95CI, PRS.P, PRS.FDR,
                 endpoint, HR, HR.95CI, HR.P, HR.FDR,
                 type, GWAS, MR, MR.95CI, MR.P, MR.FDR, MR.pleiotropy, colocalization,
                 polygenicity, LD_blocks, PolyMR, PolyMR.95CI, PolyMR.P, PolyMR.pleiotropy)]
