...
batch1 <- fread("data/INTERVAL/pre_qc_data/somalogic_proteomics/raw_data/rawData/SOMAS_r1_Raw_data_for_adam.tsv")
batch2 <- fread("data/INTERVAL/pre_qc_data/somalogic_proteomics/raw_data/rawData/SOMAS_r2_Raw_data_for_adam.tsv")
...
covar_assocs <- prot_info[,.(SeqId, Gene=Gene.Name, Protein=TargetFullName, UniProt=UniProt.Id.Current.at.Uniprot)][covar_assocs, on = .(SeqId=Seq.ID)]


# Load associations to provide some sort of ordering:
assocs <- fread("analyses/pub/cardiometabolic_proteins/all_assocs.tsv")
assocs <- assocs[, .(minFDR = min(Prot.FDR)), by=.(Gene, UniProt, Protein=Target)][order(minFDR)]
covar_assocs <- covar_assocs[assocs[,.(Gene, Protein, UniProt)], on = .(Gene, Protein, UniProt)]

# write out
fwrite(covar_assocs, sep="\t", quote=FALSE, file=sprintf("%s/protein_covariate_assocs.tsv", out_dir))
