...
scores <- rbind(idcol="PRS",
  CAD_metaGRS=sfread("data/GRS_resources/CAD_metaGRS/grs_weights.txt.gz"),
  T2D_2018=sfread("data/GRS_resources/T2D_2018/grs_weights.txt.gz"),
  CKD_2019=sfread("data/GRS_resources/CKD_2019/grs_weights.txt.gz")
)
...
# Assign 10 MB chunks individual score names
scores[, chunk := paste0(PRS, "_chr", chrIdx, "_10MB_", floor(pos/10e6))]

# spread to wide format:
scores <- dcast(scores, rsid + effect_allele ~ chunk, value.var="weight", fill=0)
